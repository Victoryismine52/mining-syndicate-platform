...
export default function TaskDetailsDialog({ open, onOpenChange, task, onStartTask }: Props) {
  ...
  const handleStartTask = async () => {
    if (task?.id && onStartTask) {
      try {
        await onStartTask(task.id);
        onOpenChange(false); // Close dialog after successful start
      } catch (error) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg md:max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            View detailed information about this task including description, due date, assignee, and metadata.
            {task?.status && ` Current status: ${String(task.status)}.`}
          </DialogDescription>
        </DialogHeader>
...
function Info({ label, value }: { label: string; value?: React.ReactNode }) {
  return (
      <div className="rounded-lg border p-2" role="group" aria-labelledby={`label-${label.toLowerCase().replace(/\s+/g, '-')}`}>
        <div 
        ...
        <div className="text-sm" aria-describedby={`label-${label.toLowerCase().replace(/\s+/g, '-')}`}>
          {value ?? "—"}
        </div>
    </div>
  )
